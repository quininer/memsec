        ...
        |(operation, cmd_server_name, ack_server_name)| unsafe {
            ...
            loop {
                ...
                match rval {
                    ...
                    TestState::Allocate => {
                        p = Some(memsec::memfd_secret().unwrap());
                        tx_ack.send(rval).unwrap();
                    }
                    ...
                    TestState::Free => {
                        memsec::free_memfd_secret(p.unwrap());
                        tx_ack.send(rval).unwrap();
                        return;
                    }
                }
...
#[cfg(unix)]
#[test]
fn memfd_secret_probe_outside_limits_canary() {
...
#[cfg(unix)]
#[test]
fn memfd_secret_probe_outside_limits_page_above() {
    attempt_write_in_region(
        Offset::SubPages(1),
        false,
...
#[cfg(unix)]
#[test]
fn memfd_secret_probe_outside_limits_two_pages_above() {
    attempt_write_in_region(
        Offset::SubPages(2),
        false,
        vec![TestState::Init, TestState::Allocate, TestState::Operation],
...
#[cfg(unix)]
#[test]
fn memfd_secret_probe_outside_limits_page_after_above() {
    attempt_write_in_region(
        Offset::AddPages(1),
        false,
        vec![TestState::Init, TestState::Allocate, TestState::Operation],
